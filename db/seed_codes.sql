-- seed_codes.sql (MariaDB)
-- code_group / code_item 초기 데이터 세트
-- 주의: code_item은 code_group_id를 FK로 참조하므로 group_key로 먼저 조회 후 INSERT 합니다.
...
-- =========================
-- 2) code_item 유틸: group_key -> code_group_id 조회를 반복하지 않도록 변수로 저장
-- =========================
-- MariaDB에서는 SELECT ... INTO 변수를 사용합니다.
...
-- =========================
-- 6) EVENT_STATUS (선택이지만 대시보드/필터에 유용)
-- =========================
INSERT INTO code_item (code_group_id, item_key, name, sort_order, is_active) VALUES
...
-- =========================
-- 7) EVENT_TYPE (초기 샘플 세트)
-- 실제 운영에서는 계속 늘어날 수 있으므로 최소만 넣고 확장 가능하게 둡니다.
-- =========================
INSERT INTO code_item (code_group_id, item_key, name, sort_order, is_active) VALUES
...
-- (선택) 확인 쿼리
-- SELECT g.group_key, i.item_key, i.name, i.sort_order
-- FROM code_group g
-- JOIN code_item i ON i.code_group_id = g.code_group_id
-- ORDER BY g.group_key, i.sort_order;